...
ILubyte *rgba = new ILubyte[m_TextData.nWidth*m_TextData.nHeight*4];
rgba = ilGetData();
///ilTexImage(m_TextData.nWidth, m_TextData.nHeight, 1, 4, IL_RGBA, IL_UNSIGNED_BYTE, rgba);

	   ilSaveImage("../../out.png");

   unsigned int width = m_TextData.nWidth;
   unsigned int height = m_TextData.nHeight;

 	for (int i = 0; i < height; i++){
	   for (int j = 0; j < width; j++){

			ILubyte red = m_TextData.pData[(i*width + j)*4 + 0];
			ILubyte green = m_TextData.pData[(i*width + j)*4 + 1];
			ILubyte blue = m_TextData.pData[(i*width + j)*4 + 2];
			ILubyte alpha = m_TextData.pData[(i*width + j)*4 + 3];
			int x = 5;
			//
     // printf( "%s\n", "Red Value for Pixel");
     // printf( "%d\n", bytes[(i*width + j)*4 + 0]); 
