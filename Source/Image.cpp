...
void CImage::SnkyBlit(int iXOffset, int iYOffset, unsigned int xFrame) const
{
	if (m_pTexture)
	{
		m_pTexture->Bind();
		...
		float Top	 = ((float)m_rectTextCoord.m_Top)/m_pTexture->GetHeight();
		float Bottom = ((float)m_rectTextCoord.m_Bottom)/m_pTexture->GetHeight();
		float Left   = ((float)m_rectTextCoord.m_Left+xFrame)/m_pTexture->GetWidth();
		float Right  = ((float)m_rectTextCoord.m_Right+xFrame)/m_pTexture->GetWidth();

		// Draw the textured rectangle.

		glBegin(GL_QUADS);
		glTexCoord2f(Left,Top);		glVertex3i(iXOffset,iYOffset,-64);
		glTexCoord2f(Left,Bottom);	glVertex3i(iXOffset,iYOffset+m_rectTextCoord.GetHeight(),-64);
		glTexCoord2f(Right,Bottom);	glVertex3i(iXOffset+m_rectTextCoord.GetWidth(),iYOffset+m_rectTextCoord.GetHeight(),-64);
		glTexCoord2f(Right,Top);	glVertex3i(iXOffset+m_rectTextCoord.GetWidth(),iYOffset,-64);
		glEnd();
	}
}
